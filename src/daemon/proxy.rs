...
impl P2PProxy {
    ...
    pub fn connect() -> Result<Self> {
        ...
        let local_peer_id = match resp {
            DaemonResponse::LocalPeerId(id) => id
                .parse()
                .map_err(|_| AppError::Io(std::io::Error::other("Invalid peer ID")))?,
    ...
    pub async fn list_remote_zones(&self, peer_id: PeerId) -> Result<Vec<String>> {
        match self.request(DaemonRequest::ListRemoteZones {
    ...
    pub async fn rs_list(&self, peer_id: PeerId) -> Result<Vec<RsFileEntry>> {
        match self.request(DaemonRequest::RsList {
            peer_id: peer_id.to_string(),
        })? {
