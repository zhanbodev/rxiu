...
use super::protocol::{DAEMON_PORT, DaemonRequest, DaemonResponse, encode_message};
...
pub async fn run_server(
    ...
    let _renew_manager = super::renew::RenewManager::start(p2p.clone());
...
async fn handle_request(
    ...
    match request {
        ...
        DaemonRequest::ListRemoteZones { peer_id } => match peer_id.parse() {
        ...
        DaemonRequest::ListRemoteFiles { peer_id, zone } => match peer_id.parse() {
        ...
        DaemonRequest::RsList { peer_id } => match peer_id.parse() {
        ...
        DaemonRequest::RsAnnounce { peer_id, file } => match peer_id.parse() {
        ...
        DaemonRequest::RsGetMeta { peer_id, name } => match peer_id.parse() {
        ...
        DaemonRequest::RsGetBlock { peer_id, hash } => match peer_id.parse() {
        ...
        DaemonRequest::RsGetBlocks { peer_id, hashes } => match peer_id.parse() {
        ...
        DaemonRequest::RsHave { peer_id, name } => match peer_id.parse() {
        ...
        DaemonRequest::RsDelete { peer_id, name } => match peer_id.parse() {
        ...
        DaemonRequest::RefreshPeers => match p2p.refresh_peers().await {
