...
pub enum NodeEvent {
    ...
    FilesReceived { peer_id: PeerId, zone: String, files: Vec<crate::storage::FileMetadata> },
    /// Response to FetchFile.
    FileReceived { name: String, content: Vec<u8>, hash: String },
    ...
    RsFilesReceived { peer_id: PeerId, files: Vec<RsFileEntry> },
    /// RS: file metadata.
    RsMetaReceived { file: RsFileEntry },
    /// RS: block data.
    RsBlockReceived { block: RsBlock },
    /// RS: multiple block data.
    RsBlocksReceived { request_id: request_response::OutboundRequestId, blocks: Vec<RsBlock> },
    /// RS: block availability.
    RsHaveReceived { peer_id: PeerId, name: String, hashes: Vec<String> },
    /// RS: ack.
    RsOk,
    /// Peer exchange: list of peers from remote.
    PeersReceived { from: PeerId, peers: Vec<crate::p2p::protocol::PeerEntry> },
    /// Response-level error.
    ResponseError { request_id: request_response::OutboundRequestId, message: String },
    /// Error occurred.
    Error(String),
}
...
impl P2PNode {
    ...
    pub fn new() -> crate::Result<Self> {
        ...
        let swarm = libp2p::SwarmBuilder::with_new_identity()
            ...
            .with_behaviour(|key| {
                ...
                NodeBehaviour { mdns, file_transfer }
            })
            .map_err(|e| std::io::Error::other(e.to_string()))?
    ...
    pub fn send_request(&mut self, peer_id: PeerId, request: FileRequest) -> request_response::OutboundRequestId {
        let request_id = self.swarm.behaviour_mut().file_transfer.send_request(&peer_id, request);
        self.pending_requests.insert(request_id, peer_id);
        request_id
    }

    /// Send a response to a pending request.
    pub fn send_response(&mut self, channel: ResponseChannel<FileResponse>, response: FileResponse) {
        let _ = self.swarm.behaviour_mut().file_transfer.send_response(channel, response);
    }
                    ...
                    match event {
                        mdns::Event::Discovered(peers) => {
                            for (peer_id, addr) in peers {
                                ...
                                let info = self.discovered_peers
                                    .entry(peer_id)
                                ...
                                request_response::Message::Response { request_id, response } => {
                                    ...
                                    return Some(match response {
                                        ...
                                        FileResponse::Zones(zones) => {
                                            NodeEvent::ZonesReceived { peer_id: peer, zones }
                                        }
                                        FileResponse::Files { zone, files } => {
                                            NodeEvent::FilesReceived { peer_id: peer, zone, files }
                                        }
                                        ...
                                        FileResponse::RsFiles(files) => {
                                            NodeEvent::RsFilesReceived { peer_id: peer, files }
                                        }
                                        ...
                                        FileResponse::RsHave(RsHave { name, hashes }) => {
                                            NodeEvent::RsHaveReceived { peer_id: peer, name, hashes }
                                        }
                                        FileResponse::RsOk => NodeEvent::RsOk,
                                        FileResponse::Peers(peers) => {
                                            NodeEvent::PeersReceived { from: peer, peers }
                                        }
                                        FileResponse::Error(msg) => {
                                            NodeEvent::ResponseError { request_id, message: msg }
                                        }
                                    });
                                }
                            }
