...
fn render_header(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let left = format!(" RXIU {}",BUILD_VERSION.to_string());
    ...
    let header_text = format!(
        ...
        left,
        right,
        width = area.width as usize - right.len()
    );

    let header =
        Paragraph::new(header_text).style(Style::default().bg(theme_color).fg(Color::Black));
...
fn render_status(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let status_text = if app.transfer.active {
        ...
        format!(
            " {} {} {} / {} {} {}%",
            if app.transfer.transfer_type == "download" {
                "⬇"
            } else {
                "⬆"
            },
    ...
    let status_bar =
        Paragraph::new(status_text).style(Style::default().bg(bg_color).fg(Color::Black));
...
fn render_browser(frame: &mut Frame, browser: &crate::ui::FileBrowser) {
    ...
    let header =
        Paragraph::new(path_text).block(Block::default().borders(Borders::ALL).title(" Navigate "));
    ...
    // Help
    let help =
        Paragraph::new(" j/k: navigate  Enter/l: open  h/Backspace: back  y: confirm  q: cancel")
